...
@Configuration
@EnableWs
public class MyWsConfigurerAdapter extends WsConfigurerAdapter {
	@Bean
	public ServletRegistrationBean messageDispatcherServlet(ApplicationContext applicationContext) {
		MessageDispatcherServlet servlet = new MessageDispatcherServlet();
		servlet.setApplicationContext(applicationContext);
		servlet.setTransformWsdlLocations(true);
		return new ServletRegistrationBean(servlet, "/employees/*");
	}
	@Bean(name = "employees")
	public DefaultWsdl11Definition defaultWsdl11Definition(XsdSchema articlesSchema) {
		...
		wsdl11Definition.setLocationUri("/employees");
		wsdl11Definition.setTargetNamespace("http://www.sarje.com/employees");
